function [ examArray, error_log ] = auto_import_exam( baseDir, par )
...
% anat
defpar.anat_regex_volume = '^v.*nii';
defpar.anat_tag_volume   = 'v';
defpar.anat_tag_json     = 'j';

% dwi
defpar.dwi_regex_volume  = '^v.*nii'; % f : multiple volumes (99% of cases) // s : only one volume (1% of cases, but it may happen)
defpar.dwi_tag_volume    = 'v';
defpar.dwi_tag_json      = 'j';

% func
defpar.func_regex_volume = '^v.*nii'; % f : multiple volumes (99% of cases) // s : only one volume (1% of cases, but it may happen)
defpar.func_tag_volume   = 'v';
defpar.func_tag_json     = 'j';

% fmap
defpar.fmap_regex_volume = '^v.*nii';
defpar.fmap_tag_volume   = 'v';
defpar.fmap_tag_json     = 'j';

% swi
defpar.swi_regex_volume = '^v.*nii';
defpar.swi_tag_volume   = 'v';
defpar.swi_tag_json     = 'j';

% asl
defpar.asl_regex_volume  = '^v.*nii'; % f : multiple volumes (99% of cases) // s : only one volume (1% of cases, but it may happen)
defpar.asl_tag_volume    = 'v';
defpar.asl_tag_json      = 'j';

% medic
defpar.medic_regex_volume  = '^v.*nii';
...
% Fetch all subdir
subdir = fullfile( gdir(EXAM.path, 'SCANS', par.serie_regex) , 'NIFTI');
if isempty(subdir)
...
% param_struct = struct;
% for j = 1 : length( json )
%    content = spm_jsonread( deblank(json{j}(1,:)) );
%    param_struct = catstruct( param_struct, content);
% end
